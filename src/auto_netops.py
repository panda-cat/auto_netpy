...
def get_device_vendor(device_type: str) -> str:
    ...
    vendor_mapping = {
        ...
        'hp': ['hp_', 'aruba', 'comware', 'procurve', 'h3c'], # 包含h3c
        'fortinet': ['fortinet'],
...
def post_connection_setup(conn: netmiko.BaseConnection, device_type: str, vendor: str, secret: Optional[str]) -> None:
    """
    **连接后设备特定设置**
    对于 Huawei/HP/H3C 等设备，即使不需要 enable，Netmiko 抽象层也可能尝试发送 enable 命令。
    使用 try/except 来捕获并忽略这些预期的连接/认证失败，以保持连接。
    """
    try:
        # Enable模式设备
        ...
        if secret and (vendor in enable_required_vendors or any(t in device_type for t in enable_required_types)):
            try:
                ...
                # 捕获预期的 enable 失败（如 Huawei/H3C/HP/Juniper 已在特权模式或无需 enable）
                # 保持连接，继续执行命令
                pass
            except Exception as e:
                # 捕获其他意外错误
                print(f"[WARN] {conn.host} Unexpected error during enable: {str(e)}. Continuing...")
                pass
...
def log_error(ip: str, error: str) -> None:
    ...
    with write_lock:
        try:
            ...
            print(f"FATAL ERROR writing log for {ip}: {str(e)}")
...
**使用方法**:
  python myapp_scripts.py -i <设备清单.xlsx> [选项]
...
**示例**:
  # 基本使用
  python myapp_scripts.py -i devices.xlsx
  
  # 配置模式 + 调试
  python myapp_scripts.py -i devices.xlsx -cs --debug
  
  # 查看支持的设备类型
  python myapp_scripts.py --list-devices
""")
        sys.exit(0)
